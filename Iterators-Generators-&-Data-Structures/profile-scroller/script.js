...
function* createPeopleIterator() {
  let index = 0;
  while (true) {
    yield people[index++ % people.length]; // % modulus operator, returns the division remainder
  }
}
...
nextBtn.addEventListener('click', ()=> {
  const person = iterator.next().value; // pulls the value of each person
  img.src = person.imageURL; // changes the img src to that persons imageURL
